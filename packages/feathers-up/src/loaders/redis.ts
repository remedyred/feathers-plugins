...
export default async function (app: Application, config: RedisClientOptions | RedisClientOptions[]) {
	if (!app.get('redisClient')) {
		app.out.verbose('Connect to redis databases...')
		if (Array.isArray(config)) {
			for (let redisDb of config) {
				await makeRedisClient(app, redisDb, redisDb.keyPrefix)
			}
		} else {
			await makeRedisClient(app, config)
		}
	}
...
async function makeRedisClient(app: Application, options: RedisClientOptions, prefix?: string) {
	const Redis = await loadRedis()

	app.set('redisClient' + (prefix ? `.${options.keyPrefix}` : ''), new Redis(options))
}

async function loadRedis(): Promise<any> {
	if (!_Redis) {
		const Redis = await import('ioredis')
		_Redis = Redis.default
	}
	return _Redis
}
